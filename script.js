...
async function translateAllWithGemini() {
    ...
    if (btn) {
        ...
        showToast('⌛ Đang tự động dịch bằng Gemini...');
    }

    try {
        const words = item.vocabData.map(r => r.from);
        const prompt = `Dịch sang tiếng Việt (mảng JSON): ${words.join(', ')}. 
Cấu trúc mỗi phần tử:
{
  ...
  "exVi": "dịch ví dụ"
        ...
        // Nếu có model cũ đã chạy được, thử nó trước tiên
        const listToTry = [];
        if (cachedModel && cachedVer) {
            listToTry.push({ model: cachedModel, ver: cachedVer });
        }
        // Thêm các model khác vào danh sách chờ (loại bỏ trùng lặp nếu cần)
        models.forEach(m => {
            versions.forEach(v => {
        ...
        if (Array.isArray(results)) {
            ...
            saveTree();
            renderVocabTable(item.vocabData);
            showToast('✨ Đã dịch tự động hoàn tất!');
        } else {
...
async function testGeminiConnection() {
    ...
    resDiv.innerHTML = '⌛ Đang kiểm tra...';
    ...
    try {
        ...
        if (names.length === 0) {
            resDiv.innerHTML = '❌ Kết nối được nhưng Key này không có model nào khả dụng.';
            resDiv.style.color = '#e74c3c';
        } else {
            resDiv.innerHTML = `✅ Kết nối thành công!<br><b>Model khả dụng:</b> <br>${names.slice(0, 10).join(', ')}${names.length > 10 ? '...' : ''}`;
            resDiv.style.color = '#27ae60';
            console.log('Full models list:', names);
        }
    } catch (e) {
        resDiv.innerHTML = `❌ Lỗi: ${e.message}<br><small>Hãy đảm bảo Key đúng và bạn không dùng VPN/Proxy bị chặn.</small>`;
        resDiv.style.color = '#e74c3c';
    }
}

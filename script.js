...
async function translateAllWithGemini() {
    ...
    // Thêm hiệu ứng shimmer cho các ô đang chờ dịch
    const rows = document.querySelectorAll('#vocabBody tr');
    rows.forEach(tr => {
        const fromCell = tr.querySelector('div[data-field="from"]');
        if (fromCell && fromCell.innerText.trim()) {
            const cellsToShimmer = tr.querySelectorAll('div[data-field]:not([data-field="from"])');
            cellsToShimmer.forEach(cell => {
                if (!cell.innerText.trim()) {
                    cell.classList.add('translating');
                }
            });
        }
    });
    ...
    try {
        ...
        // Danh sách các cặp model-version hợp lệ và ổn định nhất
        // Danh sách các cặp model-version dựa trên danh sách chuẩn mà người dùng đã cung cấp
        const listToTry = [
        ...
        // Nếu có model đã chạy thành công trước đó (cached), đưa nó lên đầu danh sách
        if (cachedModel && cachedVer) {
            const idx = listToTry.findIndex(item => item.model === cachedModel && item.ver === cachedVer);
            if (idx > -1) {
                const [cached] = listToTry.splice(idx, 1);
                listToTry.unshift(cached);
            }
        }
        ...
        for (const step of listToTry) {
            ...
            isQuotaIssue = false; // Reset lại mỗi lần thử model mới
            try {
                console.log(`📡 Trying ${model} (${ver})...`);
                const response = await fetch(`https://generativelanguage.googleapis.com/${ver}/models/${model}:generateContent?key=${key}`, {
                    ...
                    body: JSON.stringify({
                        ...
                        generationConfig: {
                            ...
                            temperature: 0.1,
                            maxOutputTokens: Math.max(2048, words.length * 300),
                            candidateCount: 1
                        }
                    })
                });
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429) {
                        isQuotaIssue = true;
                        lastError = `Hạn mức ${model}: ${errMsg}`;
                        // KHÔNG break ở đây vì quota thường tính theo model. 2.0 hết nhưng 1.5 có thể vẫn còn.
                    } else {
                        lastError = errMsg;
                    }
                    
                    // Nếu lỗi nặng như không tồn tại model, xóa cache để lần sau thử lại từ đầu
                    if (model === cachedModel) {
                        settings.workingModel = null;
        ...
        if (!successResult) {
            // Nếu lỗi cuối cùng là 429 thì mới báo lỗi Hạn Mức
            if (isQuotaIssue) throw new Error(`⚠️ Lỗi Hạn Mức: ${lastError}`);
            throw new Error(`Dịch thất bại. Lỗi: ${lastError}`);
        }
        
        let contentText = successResult;
        
        // Cố gắng extract mảng JSON từ text trả về nếu có văn bản thừa
        try {
            // Loại bỏ markdown wrapper nếu có
            contentText = contentText.replace(/```json/g, '').replace(/```/g, '').trim();
            
            // Xử lý các ký tự xuống dòng lạ bên trong chuỗi JSON có thể gây lỗi Unterminated string
            // (Chỉ giữ lại \n nếu nó nằm sau dấu phẩy hoặc ngoặc, còn lại xóa bớt space thừa)
            ...
            if (firstBracket !== -1 && lastBracket !== -1) {
                contentText = contentText.substring(firstBracket, lastBracket + 1);
            }
            ...
            if (Array.isArray(results)) {
                ...
                console.error('Gemini Return raw:', successResult);
                throw new Error('Dữ liệu trả về không đúng định dạng mảng JSON.');
            }
        } catch (parseErr) {
            console.error('JSON Parse Error. Full text length:', successResult.length);
            console.log('Preview Start:', successResult.substring(0, 200));
            console.log('Preview End:', successResult.substring(successResult.length - 200));
            console.error(parseErr);
            throw new Error(`Lỗi parse JSON: ${parseErr.message}. Hãy thử dịch lại hoặc rút ngắn danh sách từ.`);
        }
    } catch (e) {
        console.error(e);
        showToast('❌ ' + e.message);
    } finally {
        // Xóa hiệu ứng shimmer nếu còn sót lại (trong trường hợp lỗi)
